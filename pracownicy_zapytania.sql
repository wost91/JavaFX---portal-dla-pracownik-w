...
CREATE VIEW statystyki AS
    ...
    d.budzet,
    d.liczba_pracownikow,
    ROUND((d.budzet / d.liczba_pracownikow), 2) AS budzet_na_pracownika,
    ROUND(AVG(sp.pensja), 2) AS srednia_pensja,
    SUM(sp.pensja) AS suma_pensji,
    ROUND(SUM(sp.pensja) / d.budzet, 2) AS udzial_pensji_w_budzecie
FROM
    Dzial AS d,
    Pracownik_Dzial AS pd,
    Pracownicy AS p,
...
CREATE VIEW tytul_a_pensja AS
    SELECT 
        tytul, COUNT(*) as liczebnosc, ROUND(AVG(sp.pensja), 2) as srednia
    FROM
        Pracownicy AS p,
        Wykształcenie AS w,
        Pracownik_Stanowisko AS ps,
...
#tworzę widok z głównymi danymi danymi dla jednego pracownika
CREATE VIEW dane_pracownik AS
    SELECT 
        p.ID,
        p.imie,
        p.nazwisko,
